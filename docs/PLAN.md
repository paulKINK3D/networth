...
- Monthly spending drill-down showing every complete month used by the median, its scheduled/unscheduled split, included category totals, and excluded category list.
...
**Shipped extensions beyond the v1 spec above:** the cash projector maintains a known-commitment ledger and a projected cash curve. The main chart focuses on projected cash; scheduled detail remains in the event timeline and account checks. Estimated monthly spending is the median of complete monthly external outflows from up to 365 days of cash and funded-card activity, including scheduled spending. The projected curve adds only the median unscheduled remainder because scheduled cash flows are already present in Known Commitments. This prevents both one unusually expensive month from dominating and known spending from being counted twice. Split categories, internal transfers, and the durable per-category exclusion list are honored. Short histories without a complete month temporarily use an equivalent daily-average fallback.
...
- **5-tab structure** (Accounts moved behind Net Worth 2026-08-02; Goals added 2026-08-07):
  1. **Net Worth** — current total, historical chart, breakdown by account type, manual assets list; Accounts lives behind an "All Accounts" card.
  ...
  5. **Goals** — long-term savings targets and sinking funds backed by a reserve pool of dedicated savings accounts; per-goal progress, plan sufficiency, confirm-first contributions, explicit goal purchases.
- **Settings** is opened from a sheet behind the Net Worth toolbar (token entry, Face ID toggle, sync controls, manual asset CRUD) — not a tab. This uses the last tab-bar slot; a sixth tab would trigger the system More tab.
...
- **2026-08-05** — Spending remains the product and navigation label, but its report answers the broader question “where did the money go?” Spending groups are entirely user-defined: the user creates groups and assigns individual categories; Networth seeds no destination names. YNAB group identities are reference metadata only and never become Spending columns or cleanup work. Assigned and Unassigned lists show only categories used by stored transactions, and after the completed Plaid review Networth removes true category orphans while retaining split, durable-decision, and recurring-expectation references. Net Savings Transfers and Investment Contributions are Networth-owned automatic categories that use the same assignment workflow, while credit-card payments and duplicate transfer counterparts remain excluded. Savings/investment allocations remain distinct from ordinary spending in projection math, and internal reporting roles are not exposed as user-facing controls.
