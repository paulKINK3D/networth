...
**Active branch:** `feature/plaid-integration`. Plaid work is scoped to optional investment accounts and holdings delivered through a private backend. It will not replace YNAB, feed cash projections, or import ordinary bank transactions. New Plaid accounts remain excluded from Net Worth until the user resolves possible duplication with YNAB accounts or manual assets; account balances reconcile totals and holdings explain composition.

The Plaid iOS path is implemented and validated on-device in Sandbox. LinkKit 7.0.3 opens from Settings, exchanges its short-lived public token through the private backend, syncs Items/accounts/securities/holdings independently from YNAB, supports unlinking, and requires explicit per-account duplicate review. The user completed Link, received an account in Needs Review, marked it as a separate account, and confirmed its holdings render in Investments. Only reviewed, separate USD account balances contribute to Investments, current Net Worth, and daily snapshots; holdings remain explanatory detail. The backend bearer token is entered once and stored in iCloud Keychain. The Link session is retained until success or exit, and backend requests time out after 20 seconds so the connection sheet cannot remain stuck indefinitely. App-level coverage was added but not run because device builds remain user-owned; the user confirmed the app builds, Swift parsing passes, and the 52-test `NetworthCore` suite passes.

`PlaidWorker/` is deployed in Sandbox to the personal BlueLava Cloudflare account as `networth-plaid-worker`, with encrypted Item tokens in its dedicated KV namespace. The stable custom domain is `https://networth-plaid.bluelava.me`; Plaid has accepted `https://networth-plaid.bluelava.me/plaid/oauth`, the AASA route is live, and the authenticated Link-token endpoint returned HTTP 200 during the device investigation. Worker secrets remain outside git in Cloudflare and ignored local `.dev.vars`. Six Worker tests, TypeScript checking, and the deployment bundle pass. Next session: remove the Sandbox Item if it is still connected, verify unlink cleanup, then deliberately replace the Sandbox secret and environment with Production/Trial credentials and link one real investment institution.
...
- Single ModelContainer with two ModelConfigurations:
  - `NetworthLocalCache` (no CloudKit) — YNAB cache rows plus `CachedPlaidItem`, `CachedPlaidAccount`, `CachedPlaidSecurity`, and `CachedPlaidHolding`.
  - `NetworthDurable` (CloudKit private DB) — manual assets, snapshots, user/projection settings, and `DurablePlaidAccountTreatment` reconciliation decisions.
